...
const createUser = async (req, res) => {
    ...
    if (!name || !email || !mobile || !username || !password) {
        return res
            .status(401)
            .send(
                `Some fields are missing!: ${mobile}, ${username}, ${password}, ${name}, ${email}`
            );
    }
    ...
    try {
        ...
        return res
            .status(500)
            .send(
                "There was a problem creating  your account, please try again 1"
            );
    }
    ...
    if (!tempUser) {
        return res
            .status(400)
            .send(
                "There was a problem creating  your account, please try again 2"
            );
    }

    const url = `${
        currentUrl + "/user/verify/" + tempUser._id + "/" + verificationCode
    }`;
    console.log(url);
    ...
    try {
        ...
        return res
            .status(500)
            .send(
                "There was a problem sending the verification code. Please try again 3"
            );
    }
    ...
    const mail = {
        ...
        html:
            '<p><h3 style="color: green">Dear ' +
            tempUser.name +
            ", </h3>Thanks for signing up on " +
            currentUrl +
            "</p><p>Click on the link below to verify your email</p><p><a href=" +
            url +
            ">Click Here</a></p>",
    };
...
const verifyUser = async (req, res) => {
    ...
    if (Date.now() > user.expiresAt) {
        return res
            .status(404)
            .send(
                "Verification code expired. Request for a new one by visiting https://localhost/requestVerificationCode"
            );
    }
